...
*/
    ...
    public String doUploadFile(final HttpServletRequest request, final HttpServletResponse response,
        ...
        List<String> ffs = Arrays.asList(ff1.split(","));
            ...
            try {
                if (this.fm.insert(f2) > 0) {
                    if (ffs.size() > 0) {
                        ...
                        for (String s1 : ffs) {
                            fileFeature.setFileFeatureId(UUID.randomUUID().toString());
                            fileFeature.setFileId(fileId);
                            fileFeature.setFeatureId(s1);
                            this.fileFeatureMapper.insertFileIdAndFeatureId(fileFeature);
                        }
                    }
    ...
    public String doRenameFile(final HttpServletRequest request) {
        ...
        final String areaU = request.getParameter("areaX");
        final String ffU = request.getParameter("ffX");
        List<String> ffUs = Arrays.asList(ffU.split(","));
        ...
        if (!file.getFileName().equals(newFileName)) {
            ...
            //更换国家
            if (!file.getFileCountry().equals(areaU)) {
                map.put("areaU", areaU);
                if (this.fm.updateFileNameAndCountryById(map) == 0) {
                    // 并写入日志
                    return "cannotRenameFile";
                }
                fileFeatureMapper.deleteById(file.getFileId());
                if (ffUs.size() > 0) {
                    FileFeature fileFeature = new FileFeature();
                    for (String s1 : ffUs) {
                        fileFeature.setFileFeatureId(UUID.randomUUID().toString());
            ...
            map.put("newFileName", file.getFileName());
            map.put("areaU", areaU);
            if (this.fm.updateFileNameAndCountryById(map) == 0) {
                // 并写入日志
                return "cannotRenameFile";
